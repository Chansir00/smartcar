                                  ...
                                  Mat &img, Mat &output, int &roiHeight, float &leftMissedRadius, float &rightMissedRadius)
{
    ...
    case CIRCLE_INACTIVE:
    {
        ...
        if (((leftJumpPointA.x != -1 && leftJumpPointB.x != -1 && rightJumpPointA.x != -1 && rightJumpPointB.x != -1 && rightJumpPointB.x > leftJumpPointB.x) || (rightJumpPointB.x < rightJumpPointA.x && rightJumpPointB.x > 1 && (leftJumpPointA.x != -1 || leftJumpPointB.x != -1)) || (leftJumpPointA.x < leftJumpPointB.x && leftJumpPointA.x > 1 && (rightJumpPointA.x != -1 || rightJumpPointB.x != -1)) || (rightJumpPointB.y > 30 && leftJumpPointB.y >= 30 && rightJumpPointB.x - leftJumpPointB.x > 30)) && numPoints > img_devided)
        {
            circleState = CROSSING;
        }
        else if (circleflag == 1 && leftJumpPointA.x == -1 && leftJumpPointB.x == -1 && leftMissedRadius < 0.6 && isrightJumpvalid && isleftstraight && !isrightstraight && rightMissedRadius < 0.7 && numPoints > 100)
        {
            circleState = RIGHT_CIRCLE_DETECTED;
        }
        else if (circleflag == 1 && rightJumpPointA.x == -1 && rightJumpPointB.x == -1 && rightMissedRadius < 0.6 && isrightstraight && !isleftstraight && isleftJumpvalid && leftMissedRadius < 0.7 && numPoints > 100)
        {
            circleState = LEFT_CIRCLE_DETECTED;
        }
        else if (isleftstraight && lefttrend == 1 && numPoints <= img_devided)
        {
            circleState = RIGHT_TURN;
        }
        else if (isrightstraight && righttrend == -1 && numPoints <= img_devided)
        {
    ...
    {
        gpio_set_level(BEEP, 0x1);
        if (leftJumpPointB.x == -1 || isrightstraight || isleftstraight)
        {
            lost = true; // 斑马线时车速降低
        }
    }
    ...
    {
        gpio_set_level(BEEP, 0x1);
        if (numPoints < 100 || !isrightLanecontinuous || !isleftLanecontinuous)
        {
            circleState = CIRCLE_INACTIVE;
        }
    }
                                     ...
                                     float &leftMissedRadius, float &rightMissedRadius)
{
    ...
    {
        if (whitePixels[i] < maxLeft[1]||whitePixels[i] < 60) 
            continue; // 必须比当前最大值大
                                              ...
                                              Point &pointA, Point &pointB, bool &isvalid)
{
    ...
    {
        if (pointA.x >= 60 && pointB.x >= 60 && pointA.y - pointB.y >= 3 && pointA.y >= 10 && pointB.y >= 10 && pointB.x < pointA.x)
            isvalid = true;
    }
}
                                             ...
                                             Point &pointA, Point &pointB, bool &isvalid)
{
    ...
    if (pointA_found && pointB_found)
    {
        if (pointA.x <= 100 && pointB.x <= 100 && pointA.y - pointB.y >= 3 && pointA.y >= 10 && pointB.y >= 10 && pointB.x > pointA.x)
            isvalid = true;
    }
}
