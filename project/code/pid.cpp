...
float MotionController::Err_sum(const std::vector<cv::Point> &centerline)
{
    ...
    if (total_lines > 95)
    {
        ...
        for (int i = 0; i < weight_len; ++i)
        {
            ...
            if (idx >= total_lines)
                break;
            error += (centerline[idx].x - 78) * weight_middle[i];
    ...
    else if (total_lines > 65)
    {
        ...
        for (int i = 0; i < usable_points; ++i)
        {
            ...
            error += (centerline[idx].x - 78) * weight_front[weight_idx];
...
void MotionController::init_pid(PID_Controller &pid, float Kp, float Ki, float Kd, float max_out, PID_Mode mode)
{
    ...
    if (mode == FUZZY_PID)
    {
        ...
        float uff_p_max = 25.0f, uff_d_max = 40.0f;
        for (int i = 0; i < 7; ++i)
        {
...
float MotionController::calculate_pid(PID_Controller &pid, float target)
{
    ...
    else if (pid.mode == FUZZY_PID && pid.fuzzy_initialized)
    {
        ...
        cerr <<  "ec:" << ec << endl;
        float output = 0;
        output = error * (pid.fuzzy_pd.Kp0 + pid.fuzzy_pd.A * (abs(error) * delta_kp)) + ec * pid.fuzzy_pd.Kd0; // imu963ra_gyro_z * delta_kd;
        //  输出限幅
        filtered_output = output; //* alpha + (1.0f - alpha) * filtered_output;
...
void MotionController::ackerman_diff_control(int Speed_Goal, int state)
{
    ...
    float outer_factor = 1.0f + 0.4f * (W / 2.0f) * tn / L;
